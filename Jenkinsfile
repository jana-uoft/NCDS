        ...
        script {
          try {
            ...
            withCredentials([file(credentialsId: "${getPrefix()}${SITE_NAME}", variable: 'env')]) {
              sh "cat \$env > ARCHIVE/.env 2>commandResult"
            }
      ...
      steps {
        script {
          try {
            ...
            try {
              sh "ssh root@jana19.org -t \"pm2 stop $SITE\""
              sh "ssh root@jana19.org -t \"env \$(cat /var/www/$SITE/.env) pm2 reload $SITE --update-env\""
            } catch (e) {
              sh "ssh root@jana19.org -t \"env \$(cat /var/www/$SITE/.env) pm2 start server/server.js --name $SITE\""
            }
