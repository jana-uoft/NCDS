      ...
      steps {
        script {
          try {
            ...
            try {
              sh "ssh root@jana19.org \"env \$(cat /var/www/$SITE/.env)\""
              sh "ssh root@jana19.org \"pm2 stop $SITE\""
              sh "ssh root@jana19.org \"pm2 reload $SITE --update-env\""
            } catch (e) {
              sh "ssh root@jana19.org \"pm2 start server/server.js --name $SITE\""
            }
